...
- multi-file project loading through `DarkTower.toml`, with `src/main.dt` as the entry module for `dt run` and `dt build`, and a fallback to `<project>/main.dt` when `src/` is absent
- every `src/**/*.dt` file is loaded into the program when `src/` exists
